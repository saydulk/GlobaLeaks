...
class TestModels(helpers.TestGL):
    ...
    def test_receiver_add_and_del(self):
        ...
        yield self.check_entries_count({
          'Receiver': 1,
          'User': 1
        })

        yield models.User.delete(id=receiver_id)
        check = yield models.User.test(id=receiver_id)
    ...
    @inlineCallbacks
    def test_receiver_context_reference(self):
