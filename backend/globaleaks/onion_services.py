...
@transact_sync
def configure_tor_hs(store, tid, bind_port):
    return db_configure_tor_hs(store, tid, bind_port)


def db_store_onion_service(store, tid, priv_key, hostname):
...
@inlineCallbacks
def db_configure_tor_hs(store, tid, bind_port):
    priv_key = PrivateFactory(store, tid).get_val('tor_onion_priv_key')
    ...
    if priv_key == '':
        ...
        db_store_onion_service(store, tid, ephs.private_key, ephs.hostname)
    else:
