...
def receiver_serialize_receiver(store, tstate, receiver, language):
    ret_dict = user_serialize_user(store, receiver.user, language)

    ret_dict.update({
        'can_postpone_expiration': tstate.memc.can_postpone_expiration or receiver.can_postpone_expiration,
        'can_delete_submission':   tstate.memc.can_delete_submission or receiver.can_delete_submission,
        'can_grant_permissions':   tstate.memc.can_grant_permissions or receiver.can_grant_permissions,
...
@transact
def get_receiver_settings(store, tstate, receiver_id, language):
    receiver = Receiver.db_get(store, id=receiver_id)
    return receiver_serialize_receiver(store, tstate, receiver, language)


@transact
def update_receiver_settings(store, tstate, receiver_id, request, language):
...
@transact
def perform_tips_operation(store, tstate, receiver_id, operation, rtips_ids):
...
class ReceiverInstance(BaseHandler):
    ...
    def get(self):
        ...
        """
        receiver_status = yield get_receiver_settings(self.tstate,
    ...
    def put(self):
        ...
        receiver_status = yield update_receiver_settings(self.tstate,
                                                         self.current_user.user_id,
                                                         request,
                                                         self.request.language)
...
class TipsOperations(BaseHandler):
    ...
    def put(self):
        ...
        yield perform_tips_operation(self.tstate,
                                     self.current_user.user_id,
                                     request['operation'],
                                     request['rtips'])
